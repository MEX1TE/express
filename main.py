...
async def get_products(db: Session = Depends(get_db)):
    ...
    for p_orm in products_orm:
        ...
        logger.info(f"Pydantic ProductModel for ID {p_model.id}: Name: {p_model.name}, Model image_url: '{p_model.image_url}'")
...
async def get_order_history(db: Session = Depends(get_db)):
    try:
        ...
        for order_db_obj in orders_db:
            ...
            for db_item_obj in db_order_items:
                if db_item_obj.product:
                    ...
                    logger.warning(f"Order item ID {db_item_obj.id} for order ID {order_db_obj.id} references a non-existent product ID {db_item_obj.product_id}")
        ...
        raise HTTPException(status_code=500, detail=f"Внутренняя ошибка сервера при получении истории заказов: {str(e)}")

# --- Запуск сервера ---
if __name__ == "__main__":
    import uvicorn

    # Base.metadata.create_all(bind=engine) # ЗАКОММЕНТИРОВАНО ОБРАТНО
    logger.info("Attempting to create database tables if they don't exist...") # Этот лог теперь не совсем актуален, но можно оставить
    # try:
